...
- **Next (cost / abuse):** Bulk Redirect list when a token has **Account Filter Lists Edit** (`npm run sync:bulk-redirects`) — covers www copies + `newsletter.javan.de`/`digest.javan.de` homepages. Optional permalink rate-limit / Bot Fight. Playbook: `~/Projects/*.rasok.at/CLOUDFLARE-COST-OPTIMIZATION.md`.
